...
def ensure_database(name: str):
    ...
    databases = utils.run("mysql", ["-u", "root", "-e", tpl.format(database=name)])
    if databases:
        LOG.debug("Database %r already exists.", name)
        return
    LOG.debug("Database %r does not exist. Creating...", name)
    utils.run("mysql", ["-u", "root", "-e", "CREATE DATABASE {};".format(name)])
...
def ensure_user(name, password):
    ...
    users = utils.run("mysql", ["-u", "root", "-e", tpl.format(name=name)])
    if users:
