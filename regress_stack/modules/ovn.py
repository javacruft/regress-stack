...
OVN_ENCAP_IP = utils.my_ip()
# OVSDB_CONNECTION = "unix:/var/run/openvswitch/db.sock"
OVSDB_CONNECTION = f"tcp:{utils.my_ip()}:6640"
OVNNB_CONNECTION = f"tcp:{utils.my_ip()}:6641"
OVNSB_CONNECTION = f"tcp:{utils.my_ip()}:6642"

SYSTEM_ID = "/etc/openvswitch/system-id.conf"

OVS_CTL_OPTS = f"--ovsdb-server-options='--remote=ptcp:6640:{utils.my_ip()}'"

OVN_CTL_OPTS = f"""--db-nb-addr={utils.my_ip()} \
  --db-sb-addr={utils.my_ip()} \
  --db-nb-cluster-local-addr={utils.my_ip()} \
  --db-sb-cluster-local-addr={utils.my_ip()} \
...
def _add_iptable_postrouting_rule(cidr: str, comment: str) -> None:
    ...
    found = False
    try:
        utils.run(executable, ["--check", *rule_def])
    ...
    if not found:
        ...
        utils.run(executable, ["--append", *rule_def])
