...
URL = f"http://{utils.fqdn()}:8776/v3/%(project_id)s"
SERVICE = "cinder"
    ...
    utils.cfg_set(
        ...
        *utils.dict_to_cfg_set_args(
            "ceph",
            {
                ...
                "backend_host": f"{SERVICE}@{utils.fqdn()}",
            },
        ),
    )
    utils.sudo("cinder-manage", ["db", "sync"], SERVICE)
    utils.restart_apache()
    utils.restart_service("cinder-scheduler")
    utils.restart_service("cinder-volume")
